...
#define LARGE_CHUNK_SIZE N / NTHREADS
...
void parallel_temp()
{
    ...
    {
#pragma omp parallel for num_threads(NTHREADS) schedule(runtime) shared(grid, new_grid)
...
int main(int argc, char *argv[]) {
    ...
    */
    ...
    printTimeForFunction(parallel_temp, "Parallel Static with Chunksize = SMALL");

    omp_set_schedule(omp_sched_static, LARGE_CHUNK_SIZE);
    printTimeForFunction(parallel_temp, "Parallel Static with Chunksize = LARGE");

    omp_set_schedule(omp_sched_dynamic, SMALL_CHUNK_SIZE);
    printTimeForFunction(parallel_temp, "Parallel Dynamic with Chunksize = SMALL");

    omp_set_schedule(omp_sched_dynamic, LARGE_CHUNK_SIZE);
    printTimeForFunction(parallel_temp, "Parallel Dynamic with Chunksize = LARGE");
